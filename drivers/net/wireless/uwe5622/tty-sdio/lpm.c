...
#else
	pm_wakeup_ws_event(rx_ws, jiffies_to_msecs(HZ * 5), false);
#endif
}
...
int  bluesleep_init(void)
{
	...
	tx_ws = (struct wakeup_source*)wakeup_source_create("BT_TX_wakelock");
	rx_ws = (struct wakeup_source*)wakeup_source_create("BT_RX_wakelock");
